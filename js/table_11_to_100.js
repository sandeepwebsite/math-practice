...
const generateQuestion = () => {
  const randomNumber1 = randomNumber(31, 99);
  const randomNumber2 = randomNumber(2, 9);
  const question = `${randomNumber1} x ${randomNumber2} = ?`;
...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_d1_100')) {
      localStorage.setItem('time_data_d1_100', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_d1_100'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_d1_100', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_d1_100')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_d1_100'));
...
//these two part is moved to common.js

// Reset Button Logic
function resetQuizButton() {
  // Clear the stored timer values and table data
  localStorage.removeItem('time_data_d1_100');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_d1_100')) || [];
