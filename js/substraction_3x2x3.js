...
const generateQuestion = () => {
  ...
  const question = `${randomNumber1} - ${randomNumber2} - ${randomNumber3} = ?`;
  const answer = randomNumber1 - randomNumber2 - randomNumber3;
...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_s323')) {
      localStorage.setItem('time_data_s323', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_s323'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_s323', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_s323')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_s323'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_s323');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_s323')) || [];
