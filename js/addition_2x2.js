...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_a22')) {
      localStorage.setItem('time_data_a22', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_a22'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_a22', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_a22')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_a22'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_a22');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_a22')) || [];
