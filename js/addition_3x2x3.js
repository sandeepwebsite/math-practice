...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_a323')) {
      localStorage.setItem('time_data_a323', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_a323'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_a323', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_a323')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_a323'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_a323');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_a323')) || [];
