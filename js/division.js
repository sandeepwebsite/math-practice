...
const generateQuestion = () => {
  const randomNumber1 = randomNumber(2, 1000);
  const randomNumber2 = randomNumber1 * randomNumber(10, 100);
  // const randomNumber3 = randomNumber(100, 999);
  const question = ` ${randomNumber2} ÷ ${randomNumber1} = ?`;

  const answer = randomNumber2 / randomNumber1;
...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_division')) {
      localStorage.setItem('time_data_division', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_division'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_division', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_division')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_division'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_division');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_division')) || [];
