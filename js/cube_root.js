...
const generateQuestion = () => {
  const randomNumber1 = randomNumber(2, 50);
  const randomNumber2 = randomNumber1 * randomNumber1 *randomNumber1;
  // const randomNumber3 = randomNumber(1, 3);
  const question = `∛ ${randomNumber2} = ?`;

  const answer = Math.cbrt(randomNumber2);
...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_cbrt_2_20')) {
      localStorage.setItem('time_data_cbrt_2_20', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_cbrt_2_20'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_cbrt_2_20', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_cbrt_2_20')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_cbrt_2_20'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_cbrt_2_20');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_cbrt_2_20')) || [];
