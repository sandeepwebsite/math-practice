...
function prepareQuestions() {
 const available = Object.keys(articleTexts);
 questions = shuffle(available).map(key => ({
  article: key,
...
function getNearbyOptions(correct) {
  ...
  const correctIndex = articleList.indexOf(correct);

  // Search for nearest articles by index
  let offset = 1;
  while (wrongOptions.size < 3 && offset < articleList.length) {
    const leftIndex = correctIndex - offset;
    const rightIndex = correctIndex + offset;

    if (leftIndex >= 0) {
      const option = articleList[leftIndex];
      if (option !== correct) {
        wrongOptions.add(option);
      }
    }

    if (wrongOptions.size < 3 && rightIndex < articleList.length) {
      const option = articleList[rightIndex];
      if (option !== correct) {
        wrongOptions.add(option);
      }
    }
  ...
  // Fallback to random if not enough unique nearest options
...
function loadQuestion() {
 ...
 timeLeft = 60;
 timerText.innerText = 'TimeLeft: '+ timeLeft;
 timerBar.style.width = "100%";

 let current = questions[currentQ];
 questionEl.innerText = 'Article: ' + current.text;
 ...
 options.forEach((optText, index) => {
  ...
  opt.style.left = (20 + index * 20) + "%";
  opt.style.animationDelay = (Math.random() * 2) + "s";
  opt.onclick = () => handleAnswer(opt, optText, correct);
...
function handleAnswer(opt, selected, correct) {
 clearInterval(timer);
 stopOptions();
 if (selected === correct) {
  ...
  resultEl.innerHTML = "❌ Wrong! Correct: Article " + correct;
  document.querySelectorAll(".option").forEach(o => {
   if (o.innerText === correct) o.style.background = "green";
  });
 }
...
function showAnswer(correct) {
 stopOptions();
 resultEl.innerHTML = "⏰ Time's up! Correct: Article " + correct;
 document.querySelectorAll(".option").forEach(opt => {
  if (opt.innerText === correct) opt.style.background = "green";
 });
 nextBtn.style.display = "inline-block";
}
