...
const generateQuestion = () => {
  const randomNumber1 = randomNumber(31, 119);
...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_sq31')) {
      localStorage.setItem('time_data_sq31', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_sq31'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_sq31', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_sq31')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_sq31'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_sq31');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_sq31')) || [];
