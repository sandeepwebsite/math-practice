...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_t730')) {
      localStorage.setItem('time_data_t730', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_t730'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_t730', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_t730')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_t730'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_t730');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_t730')) || [];
