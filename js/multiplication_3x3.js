...
const generateQuestion = () => {
  ...
  const randomNumber2 = randomNumber(100, 999);
...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_m33')) {
      localStorage.setItem('time_data_m33', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_m33'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_m33', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_m33')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_m33'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_m33');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_m33')) || [];
