...
const nextQuestion = () => {
  ...
  if (currentQuestion <= totalQuestions) {
    ...
    if (!localStorage.getItem('time_data_m32')) {
      localStorage.setItem('time_data_m32', '[]');
    }
    let oldTime = JSON.parse(localStorage.getItem('time_data_m32'));
    oldTime.push(newTime);
    localStorage.setItem('time_data_m32', JSON.stringify(oldTime));
...
function displayTimeTable() {
  if (localStorage.getItem('time_data_m32')) {
    let timeArray = JSON.parse(localStorage.getItem('time_data_m32'));
...
function resetQuizButton() {
  ...
  localStorage.removeItem('time_data_m32');
...
function updateGraphData() {
  ...
  let graphData = JSON.parse(localStorage.getItem('time_data_m32')) || [];
